...
TWO RETRIEVAL VIEWS (plan §A Fix A — `kb-query --scope all` does NOT see all 7
collections, only shared + the caller's private one):
  1. ALL-7  : query each of the 7 collections directly via mmrag.py and MERGE by
              similarity -> the corpus-wide number (what's actually retrievable).
  2. PROD   : the real production path = shared-cortex + the query's own agent
              collection, merged -> real-world retrieval for that agent.
  The report shows BOTH; neither is presented as the other.
...
def evaluate(eval_set: dict, python: str, mmrag: str, base_env: dict,
             snapshot_dir, prod_threshold: float, query_fn=query_collection) -> dict:
    ...
    for q in eval_set["queries"]:
        ...
        all7 = merge_ranked(list(by_col.values()))
        # PROD = shared + (the query's agent collection, if it is an agent-* one).
        prod_cols = [PROD_SHARED]
        if isinstance(col, str) and col.startswith("agent-"):
            prod_cols.append(col)
        prod = merge_ranked([by_col.get(c, []) for c in prod_cols])

        all7_p, prod_p = ranked_paths(all7), ranked_paths(prod)
        per_query.append({
            "id": q.get("id"), "tag": q.get("tag"), "query": q["query"],
            "collection": col, "negative_control": negative,
            "expected_sources": fragments,
            "all7": _query_block(all7, all7_p, fragments, negative, prod_threshold),
            "prod": _query_block(prod, prod_p, fragments, negative, prod_threshold),
        })
    ...
    return {
        ...
        "aggregate": {"all7": _agg(per_query, "all7"), "prod": _agg(per_query, "prod")},
...
def _query_block(merged, paths, fragments, negative, threshold) -> dict:
    return {
        "recall@5": recall_at_k(paths, fragments, 5),
        "recall@20": recall_at_k(paths, fragments, 20),
        "rr": reciprocal_rank(paths, fragments),
        "false_positive": is_false_positive(merged, threshold) if negative else None,
        "top5": paths[:5],
    }
...
def _per_collection(per_query) -> dict:
    ...
    return {c: {"all7": _agg(rows, "all7"), "prod": _agg(rows, "prod")}
...
def _misses(per_query) -> list:
    ...
    ALL-7 view = a real fix-mapping-or-reingest signal (surface loud)."""
    return [{"id": r["id"], "tag": r["tag"], "query": r["query"],
             "collection": r["collection"], "expected_sources": r["expected_sources"]}
            for r in per_query
            if not r["negative_control"] and r["all7"]["recall@20"] == 0.0]
...
def print_report(report: dict) -> None:
    ...
    print("=" * 74)
    for view in ("all7", "prod"):
        a = report["aggregate"][view]
        label = "ALL-7 (corpus-wide)" if view == "all7" else "PROD (shared + agent-private)"
        print(f"\n[{label}]  n={a['n_scored']} scored / {a['n_controls']} controls")
        print(f"  Recall@5={a['recall@5']}  Recall@20={a['recall@20']}  MRR={a['mrr']}  "
              f"FP-rate={a['false_positive_rate']}")
    print("\nPer-collection (ALL-7):")
    for c, b in report["per_collection"].items():
        x = b["all7"]
        print(f"  {c:16s} n={x['n_scored']:2d}  R@5={x['recall@5']}  R@20={x['recall@20']}  MRR={x['mrr']}")
