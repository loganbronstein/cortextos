  ...
  .action(async (name: string, options: { template: string; org?: string; instance: string }) => {
    ...
    if (!existsSync(envPath)) {
      writeFileSync(envPath, [
        ...
        '# ALLOWED_USER: REQUIRED when BOT_TOKEN is set. Your numeric Telegram user ID',
        '# (NOT your @username). The daemon refuses to enable Telegram without it because',
        '# anyone who finds the bot @handle could otherwise control the agent. Get yours by',
        '# sending any message to @userinfobot, or check the "from.id" field in the',
        '# getUpdates response above.',
        '#',
        'BOT_TOKEN=',
        'CHAT_ID=',
        'ALLOWED_USER=',
