...
> **SECURITY FLAG:** The install command in this skill uses `pip install graphifyy` (double-y). Verify
> this is the correct package name before installing. A typosquat of the legitimate `graphify` package
> (single-y) could be a supply chain attack vector. Confirm the correct PyPI package name with the
> skill author before running the install step.
