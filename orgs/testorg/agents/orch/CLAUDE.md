...
### Create task
```bash
TASK_ID=$(cortextos bus create-task "<clear title>" "<description>" $CTX_AGENT_NAME <priority>)
...
### Complete task
```bash
cortextos bus complete-task "$TASK_ID" "<what you produced>"
```
...
**Active agents:**
- worker (specialist) - executes tasks delegated by the orchestrator
