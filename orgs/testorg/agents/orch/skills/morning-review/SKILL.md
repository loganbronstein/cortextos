...
### 5A: Check Content Calendar (PRIMARY)

Read `skills/content-calendar/SKILL.md` and run the **review** workflow:
...
slots = day.get('slots', [])
scripted = sum(1 for s in slots if s['status'] == 'scripted')
researched = sum(1 for s in slots if s['status'] == 'researched')
empty = sum(1 for s in slots if s['status'] == 'empty')
skool = day.get('skool_post', {}).get('status', 'empty')
print(f'Content: {scripted}/3 scripted | {researched}/3 researched | {empty} empty | Skool: {skool}')
"
```

Include this output line in Message 3 (Tasks) of the morning briefing.

**If any slots are empty or only researched → dispatch worker immediately:**

```bash
TASK_ID=$(cortextos bus create-task "Script content slots for $TODAY" "Read $CTX_FRAMEWORK_ROOT/orgs/$CTX_ORG/content-calendar.json. Write full hook/body/CTA scripts for today's slots. Update status to scripted." worker high)
cortextos bus send-message worker high "Urgent: script today's content calendar slots before 8:30 AM. Task ID: $TASK_ID. File: orgs/$CTX_ORG/content-calendar.json"
cortextos bus log-event action task_dispatched info --meta "{\"to\":\"worker\",\"task\":\"script content slots\",\"date\":\"$TODAY\"}"
```

### 5B: Fallback (no calendar data for today)

If today's date is missing from content-calendar.json:

```bash
cortextos bus send-message worker high "Content calendar missing today ($TODAY). Run populate workflow: read skills/content-calendar/SKILL.md and populate + research today immediately."
```

Note in briefing:
```
Content: Calendar not populated for today — worker dispatched to fix.
```
