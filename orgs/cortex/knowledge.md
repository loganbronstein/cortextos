...
- Logan's canonical knowledge baseline is the Lazy Obsidian Method in `Vault/Cortex/_org/Compound/lazy-obsidian-framework.md`: Vault=Obsidian, Method=Karpathy LLM wiki, Structure=PARA, Workflow=Kepano Obsidian Skills, Graph=Graphify, Search=QMD.
- The required Karpathy loop is literal: capture raw -> process -> link -> compound. Scribe runs this through `cortextos bus vault lazy`, with daily ingest, nightly review, and weekly vault jobs.
...
- AscendOps review 2026-04-30: reuse framework-level reliability patterns only, not property-management niche workflows. Adopted compaction fact extraction + SessionStart restore and loop-detector hooks; hold larger vendor-adapter, A2A, Supabase bus, and property-management skill imports until a separate architecture review.
