...
1. **Send boot message first** — before reading anything else. SKIP this step if your startup prompt says `CONTEXT HANDOFF` or `SILENT AUTO-RESET` (those are handoff/auto-reset restarts, not cold boots):
   ```bash
...
6a. **Safety-net presence check (HARD RULE — every session start).** The pre-send + pre-action guardrails are what stop drift. Verify they are still wired:
   ```bash
   bash /Users/loganbronstein/cortextos/orgs/cortex/agents/boss/scripts/safety-net-check.sh
   ```
   If exit 0: ok, continue. If exit 1: scripts or hook config is missing — DO NOT silently auto-repair (could mask a deliberate change). Read the failure list, restore the missing pieces from version control, then alert Logan via Telegram with `Safety net broken: <list>`. The four required pieces:
   - `scripts/telegram-guardrail.py` (executable)
   - `scripts/action-grader.py` (executable)
   - `.claude/settings.json` PreToolUse Bash hook entry referencing `telegram-guardrail.py`
   - `.claude/settings.json` PreToolUse Bash hook entry referencing `action-grader.py`
   The canonical rule list lives in the Vault at `Vault/Rules/Safety Net.md` — Logan can edit it without touching code; the scripts read banned words from there at every check. Audit log lands in `Vault/Reports/safety-net/<date>.md`.
7. Check today's memory file (`memory/$(date -u +%Y-%m-%d).md`) for any in-progress work
8. **POST-RESET ACTIVE-CHECK PROTOCOL (HARD RULE — runs before any proactive Telegram ping).** Auto-reset restores facts from memory files but NOT conversational subtext, AND does NOT verify whether in-flight deliverables landed while boss was being reset. On EVERY session start (especially silent auto-reset), before any other action:
   ```bash
   # 8a. Read the Vault — primary canonical surface for everything Cortex has captured.
   #     Daily Notes are the conversation archive, GRAPH_REPORT is the latest synthesis,
   #     Cortex Index is the dashboard for everything Cortex has put in your Vault.
   cat "/Users/loganbronstein/Sale Advisor/Vault/Cortex Index.md" 2>/dev/null | head -120
   for d in $(date -u +%Y-%m-%d) $(date -u -v -1d +%Y-%m-%d 2>/dev/null) $(date -u -v -2d +%Y-%m-%d 2>/dev/null); do
     [[ -f "/Users/loganbronstein/Sale Advisor/Vault/Daily Notes/$d.md" ]] && \
       echo "=== Daily Note $d (last 80 lines) ===" && \
       tail -80 "/Users/loganbronstein/Sale Advisor/Vault/Daily Notes/$d.md"
   done
   [[ -f "/Users/loganbronstein/Sale Advisor/Vault/graphify-out/GRAPH_REPORT.md" ]] && \
     head -80 "/Users/loganbronstein/Sale Advisor/Vault/graphify-out/GRAPH_REPORT.md"
   # Today's lint + promote queue if they exist
   [[ -f "/Users/loganbronstein/Sale Advisor/Vault/Reports/cortextos-lint/$(date -u +%Y-%m-%d).md" ]] && \
     head -40 "/Users/loganbronstein/Sale Advisor/Vault/Reports/cortextos-lint/$(date -u +%Y-%m-%d).md"
   [[ -f "/Users/loganbronstein/Sale Advisor/Vault/Reports/cortextos-promote-queue/$(date -u +%Y-%m-%d).md" ]] && \
     cat "/Users/loganbronstein/Sale Advisor/Vault/Reports/cortextos-promote-queue/$(date -u +%Y-%m-%d).md"
   # Helper agent files updated in last 3h (ship signals during reset)
   find "/Users/loganbronstein/Sale Advisor/Vault/Research/cortextos/" -type f -mmin -180 2>/dev/null | head -10

   # 8b. Check every agent heartbeat for ship signals newer than last memory entry
   cortextos bus read-all-heartbeats

   # 8c. Scan the LAST 10 outbound messages to Logan for active commitments + closer signals
   tail -20 "$CTX_FRAMEWORK_ROOT/orgs/$CTX_ORG/agents/$CTX_AGENT_NAME/outbound-messages.jsonl" 2>/dev/null
   ```
   Pattern-match the outbound tail for: "I will ping you when X", "Next ping = Y", "Silent until Z", "Waiting on W", "K"/"Ok"/"Got it"/"Cool" (closer signals).
   - If a ship signal exists that you have NOT yet acted on (e.g. Pass X landed in Vault but handoff not dispatched) → dispatch immediately. Do NOT wait for the next heartbeat cron.
   - If a closer signal exists and no fire-level blocker has emerged since → DEFAULT TO SILENT. Do not proactively ping Logan in the first 10 minutes post-reset. Only ping if (a) a prior commitment condition fires in a verifiable way, (b) Logan re-initiates, or (c) a true fire-level blocker surfaces that he has not already been told about.
9. If resuming a task, query the Vault first: `cortextos bus vault search "<task topic>" --no-rerank`. Use `kb-query` only as a fallback for non-Vault indexed org docs.
...
14. Send your online status message — **only AFTER crons are confirmed set AND only if step 8 did not land you in closer-silent mode**. On a cold boot: tell them crons running, pending messages, and what you are picking up from last session. On a `CONTEXT HANDOFF` restart: send ONE brief conversational message that picks up naturally (e.g. "back — [what you were working on]"). On a `SILENT AUTO-RESET`: skip the online message entirely; silence is the correct behavior. No cron IDs, no status report.
