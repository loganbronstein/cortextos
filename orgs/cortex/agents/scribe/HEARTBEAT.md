...
- If you have NO tasks: run the memory surface checks in Step 6, then create or execute one concrete memory task from GOALS.md
...
## Step 6: Check memory surface

Read GOALS.md. Goals are refreshed daily by the orchestrator each morning, but scribe has standing ownership even when no task is assigned.

Required checks every heartbeat:

```bash
/Users/loganbronstein/cortextos/orgs/cortex/agents/scribe/scripts/vault-sync-agent-logs.sh
node /Users/loganbronstein/cortextos/orgs/cortex/agents/scribe/scripts/vault-sync-chat-transcripts.mjs
test -f "/Users/loganbronstein/Sale Advisor/Vault/Cortex Index.md"
test -f "/Users/loganbronstein/Sale Advisor/Vault/State/Fleet.md"
```

- If today's `Vault/Daily/YYYY-MM-DD.md` is missing after 8 AM Chicago time: create it.
- If today's `Vault/Research/cortextos/chat-research/YYYY-MM-DD.md` is missing after transcript sync: run `vault-synthesize-chat-research.mjs`.
- If the latest `Reports/scribe-conflict-scan/` file is older than 24h: run a new conflict scan.
- If `Cortex Index.md` does not mention scribe, agent-log ledgers, daily briefs, and conflict scans: update it.
- If goals are stale (>24h without update): do the checks above first, then message the orchestrator to refresh goals.
- If you have no tasks: create a concrete task for the missing memory surface and execute it. Don't idle.
