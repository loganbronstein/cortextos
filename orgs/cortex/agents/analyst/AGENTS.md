...
3a. **Read Logan's Vault — canonical fleet knowledge surface (added 2026-04-26).** Cortex captures every Telegram conversation, helper memory, research output, and graph synthesis into Logan's Vault. Read these every session start so you share context with the rest of the fleet:
    ```bash
    # Cortex Index = dashboard for everything Cortex put in Logan's Vault
    cat "/Users/loganbronstein/Sale Advisor/Vault/Cortex Index.md" 2>/dev/null | head -120
    # Last 2 days of fleet conversation
    for d in $(date -u +%Y-%m-%d) $(date -u -v -1d +%Y-%m-%d 2>/dev/null || date -u -d "yesterday" +%Y-%m-%d); do
      [[ -f "/Users/loganbronstein/Sale Advisor/Vault/Daily Notes/$d.md" ]] && \
        echo "=== Daily $d (last 60 lines) ===" && \
        tail -60 "/Users/loganbronstein/Sale Advisor/Vault/Daily Notes/$d.md"
    done
    # Latest graph synthesis report (drift, hubs, surprises across the Vault)
    [[ -f "/Users/loganbronstein/Sale Advisor/Vault/graphify-out/GRAPH_REPORT.md" ]] && \
      head -60 "/Users/loganbronstein/Sale Advisor/Vault/graphify-out/GRAPH_REPORT.md"
    # Today's lint findings (broken links, stale notes — fix opportunistically)
    ...
    ```
    The Vault is the canonical surface for everything Cortex has captured. When you write claims that touch named entities, search Vault first via QMD before writing to avoid contradicting prior work.
...
8. If resuming a task, query the Vault first: `cortextos bus vault search "<task topic>" --no-rerank`. Use `kb-query` only as a fallback for non-Vault indexed org docs.
