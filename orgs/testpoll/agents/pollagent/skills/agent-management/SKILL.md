...
bash $CTX_FRAMEWORK_ROOT/scripts/setup.sh
...
bash $CTX_FRAMEWORK_ROOT/bus/soft-restart.sh <agent_name> "<reason>"

# Example:
bash $CTX_FRAMEWORK_ROOT/bus/soft-restart.sh sentinel "model change to sonnet"
```
...
bash $CTX_FRAMEWORK_ROOT/bus/hard-restart.sh --reason "context exhaustion"
```
...
bash $CTX_FRAMEWORK_ROOT/bus/soft-restart.sh donna "goal refresh"
...
bash $CTX_FRAMEWORK_ROOT/bus/soft-restart.sh "$AGENT" "model change to $NEW_MODEL"
```
...
bash $CTX_FRAMEWORK_ROOT/bus/soft-restart.sh "$AGENT" "bot token updated"
```
...
bash $CTX_FRAMEWORK_ROOT/bus/send-message.sh "$AGENT" normal \
...
### Enable
```bash
bash $CTX_FRAMEWORK_ROOT/scripts/enable-agent.sh <agent> --org <org>

# With restart (resets crash counter)
bash $CTX_FRAMEWORK_ROOT/scripts/enable-agent.sh <agent> --org <org> --restart
```

### Disable
```bash
bash $CTX_FRAMEWORK_ROOT/scripts/disable-agent.sh <agent> --org <org>
```
...
### Check All Agents
```bash
bash $CTX_FRAMEWORK_ROOT/bus/read-all-heartbeats.sh
```
...
### List All Agents
```bash
bash $CTX_FRAMEWORK_ROOT/scripts/list-agents.sh --format json
```
...
bash $CTX_FRAMEWORK_ROOT/bus/soft-restart.sh "$AGENT" "recovery from dead bash"
```
