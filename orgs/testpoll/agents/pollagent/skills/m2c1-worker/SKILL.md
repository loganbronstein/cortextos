...
```bash
# Add to existing .claude/settings.json or create it
# See "Permission Bypass for .claude/ Directory" section below for full options
python3 -c "
import json, os
path = '$PROJECT_DIR/.claude/settings.json'
cfg = json.load(open(path)) if os.path.exists(path) else {}
cfg.setdefault('permissions', {}).setdefault('allowedPaths', [])
if '.claude/' not in cfg['permissions']['allowedPaths']:
    cfg['permissions']['allowedPaths'].append('.claude/')
...
```
bash $CTX_FRAMEWORK_ROOT/bus/send-message.sh <your-agent-name> normal '<message>'
```
Check inbox:
```
bash $CTX_FRAMEWORK_ROOT/bus/check-inbox.sh
```
...
## Permission Bypass for .claude/ Directory

Workers frequently get stuck on permission prompts when writing to `.claude/orchestration-*/` and other `.claude/` subdirectories. This happens even with `--dangerously-skip-permissions` because `.claude/` is a protected directory.

### Option 1: Project-level allowed paths (RECOMMENDED)

Before spawning the worker, create or update the project's settings to allow `.claude/` writes:
...
### Option 2: Global user-level permissions

Add `.claude/` to the user-level allowed paths at `~/.claude/settings.json`:

```bash
# Check existing settings first
cat ~/.claude/settings.json 2>/dev/null

# Add .claude/ to allowedPaths if not present
# Be careful not to overwrite existing settings
```

### Option 3: Handle prompts via tmux (FALLBACK)

If permission prompts still appear despite the above, handle them via send-keys:

```bash
# If worker is stuck at a permission prompt, approve it
tmux send-keys -t "$WORKER_SESSION" "y" Enter

# Or escape and nudge the worker to use a different path
tmux send-keys -t "$WORKER_SESSION" Escape Enter
```

### Important Notes
- `--dangerously-skip-permissions` covers most file operations but `.claude/` has additional protections
- Always set up Option 1 during Phase 0 Setup, before spawning the worker
- If the worker creates subagents, those subagents inherit the project settings
...
```bash
bash $CTX_FRAMEWORK_ROOT/bus/send-message.sh <worker-name> normal '<your answers>'
```
...
```bash
bash $CTX_FRAMEWORK_ROOT/bus/send-message.sh <worker-name> normal \
...
```bash
# Log the milestone
bash $CTX_FRAMEWORK_ROOT/bus/log-event.sh milestone m2c1_complete info \
  '{"project":"<name>","location":"<path>","tasks":<count>,"tests":<count>}'

# Notify orchestrator
bash $CTX_FRAMEWORK_ROOT/bus/send-message.sh paul normal \
...
```bash
# Log what happened
bash $CTX_FRAMEWORK_ROOT/bus/log-event.sh action m2c1_failed info \
