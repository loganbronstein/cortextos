...
13. Send your online status message — **only AFTER crons are confirmed set**. On a cold boot: tell them crons running, pending messages, and what you are picking up from last session. On a `CONTEXT HANDOFF` restart: do NOT send a Telegram ping. The user did not initiate this restart — it is routine context cleanup with history fully preserved. Resume working silently. Only ping if (a) a user message arrived during the gap and you owe a reply, or (b) you have a real result to surface.
